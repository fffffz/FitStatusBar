...
public class StatusBarManager3 {
    ...
    public static ViewGroup setStatusBarColor(Activity activity, View view, int color, boolean isTextColorBlack, boolean shouldAddMaskBelowM) {
        ...
        mTextColorBlack = isTextColorBlack;
    ...
    public static boolean setStatusBarTextColor(Activity activity, boolean isTextColorBlack) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            return setStatusBarTextColorForLollipop(activity, isTextColorBlack);
        }
    }
    ...
    private static boolean setStatusBarTextColorForLollipop(Activity activity, boolean isTextColorBlack) {
        Window window = activity.getWindow();
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
            window.addFlags(WindowManager.LayoutParams.FLAG_TRANSLUCENT_STATUS);
            window.addFlags(WindowManager.LayoutParams.FLAG_DRAWS_SYSTEM_BAR_BACKGROUNDS);
            float[] hsv = new float[3];
            Color.colorToHSV(isTextColorBlack ? Color.WHITE : Color.BLACK, hsv);
            hsv[2] *= 0.8f;
